...
namespace aw_decision
{
     ...
     * @brief 创建前往A点状态
     */
    class GOTO_A_State : public SimpleState
    {
    public:
        GOTO_A_State(Position2D pos,
                     NavClient::SharedPtr nav_client,
                     std::shared_ptr<rclcpp::Node> node,
                     double timeout = 30.0)
            : SimpleState("GOTO_A", Actions::GOTO_A),
              position_(pos),
              ...
              timeout_(timeout)
        {
            ...
            this->setEnter([this]()
                ...
                {
                    RCLCPP_ERROR(node_->get_logger(), "GOTO_A: nav_client_ is nullptr!");
                    return;
                }

                if (!nav_client_->wait_for_action_server(std::chrono::seconds(3)))
                {
                    RCLCPP_ERROR(node_->get_logger(), "GOTO_A: Action server is not available after waiting 3s...");
                    return;
                }
                ...
                nav_goal_options.goal_response_callback = [this](NavGoalHandler::SharedPtr goal_handle)
                {
                    ...
                    if (!goal_handle_)
                    {
                        RCLCPP_ERROR(node_->get_logger(), "GOTO_A: 目标点被拒绝");
                        is_failed_ = true;
                        return;
                    }
                    ...
                    {
                        is_goal_sent_ = true;
                        RCLCPP_INFO(node_->get_logger(), "GOTO_A: 目标点已发送");
                    }
                };
                ...
                nav_goal_options.result_callback = [this](const NavGoalHandler::WrappedResult &result)
                {
                    ...
                    case rclcpp_action::ResultCode::SUCCEEDED:
                        RCLCPP_INFO(node_->get_logger(), "GOTO_A: 导航成功");
                        is_arrived_ = true;
                        break;

                    case rclcpp_action::ResultCode::ABORTED:
                        RCLCPP_ERROR(node_->get_logger(), "GOTO_A: 导航失败");
                        is_failed_ = true;
                        break;

                    case rclcpp_action::ResultCode::CANCELED:
                        RCLCPP_INFO(node_->get_logger(), "GOTO_A: 导航被取消");
                        is_failed_ = true;
                        break;

                    default:
                        RCLCPP_ERROR(node_->get_logger(), "GOTO_A: 未知状态");
                        is_failed_ = true;
                        break;
                    }
                };
                RCLCPP_INFO(node_->get_logger(), "GOTO_A: 导航开始");
            ...
            this->setExec([this]() -> Actions
                ...
                {
                    RCLCPP_INFO(node_->get_logger(), "GOTO_A: 等待目标点发送确认...");
                    return Actions::GOTO_A;  // 继续保持在当前状态
                }
                
                // 2. 超过缓冲时间仍未发送，说明发送失败
                if (!is_goal_sent_)
                {
                    RCLCPP_ERROR(node_->get_logger(), "GOTO_A: 目标点发送失败");
                ...
                if (elapsed > timeout_)
                {
                    RCLCPP_ERROR(node_->get_logger(), "GOTO_A: 超时！已耗时 %.1f 秒", elapsed);
                    return Actions::DEFAULT;
                }
                ...
                if (is_failed_)
                {
                    RCLCPP_ERROR(node_->get_logger(), "GOTO_A: 导航失败，准备退出状态");
                    return Actions::DEFAULT;
                }
                ...
                if (is_arrived_)
                {
                    RCLCPP_INFO(node_->get_logger(), "GOTO_A: 成功到达目标点");
                    return Actions::DEFAULT;
                }
                
                // 6. 还在导航中
                RCLCPP_INFO(node_->get_logger(), "GOTO_A: 导航中... 已耗时 %.1f 秒", elapsed);
                return Actions::GOTO_A; });
            ...
            if (goal_handle_ && !is_arrived_ && !is_failed_)
            {
                ...
                RCLCPP_INFO(node_->get_logger(), "GOTO_A: 退出状态，取消导航");
            }
            else
            {
                RCLCPP_INFO(node_->get_logger(), "GOTO_A: 退出状态");
            } });
            ...
            this->setReset([this]()
                ...
                {
                    nav_client_->async_cancel_goal(goal_handle_);
                    RCLCPP_INFO(node_->get_logger(), "GOTO_A: 重置状态，取消导航");
                }
                else if (goal_handle_)
                {
                    RCLCPP_INFO(node_->get_logger(), "GOTO_A: 重置状态（导航已结束）");
                }
                
                goal_handle_.reset(); });
        }

        ~GOTO_A_State() = default;
