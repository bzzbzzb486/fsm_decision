...
namespace aw_decision
{
     ...
     * @brief 一次性状态类
     */
    class PATTERN_MOVE : public SimpleState
    {
    public:
        PATTERN_MOVE(std::shared_ptr<rclcpp::Node> node,
                     std::shared_ptr<ResourceHub> resource_hub,
                     std::shared_ptr<PatternType> pattern)
            : SimpleState("Pattern_Move", Actions::MOVE_PATTERN),
              _node(node),
              _resource_hub(resource_hub),
              _pattern(Pattern(PatternType::MOVE)),
              _current_pattern(pattern)
        {
            ...
            setExec([this]()
                    {   RCLCPP_INFO(this->_node->get_logger(), "PATTERN_MOVE: 已切换为移动姿态");
                        return Actions::DEFAULT; });

            setExit([this]() {});

            setReset([this]()
                     { RCLCPP_ERROR(this->_node->get_logger(), "PATTERN_MOVE: 未定义的行为"); });

            setInterrupt([this]()
                         { return false; });
        }

        ~PATTERN_MOVE() = default;
